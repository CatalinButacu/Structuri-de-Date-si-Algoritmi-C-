...
int HashTable::hash(char* cheie)
{
	int valHash = 0;
	for (int i = 0; i < (int)strlen(cheie); ++i) //calculeaza suma codurilor ASCII
	{
		valHash += (int)cheie[i];
	}
	return valHash%lungime; //se returneaza restul impartirii sumei codurilor ASCII la lungimea tabelului Hash
}

void HashTable::inserareNod(Nod* nodNou)
{
	int index = hash(nodNou->data); //calculam un index pentru noul element de adugat folosind functia hash()
...
{
	int index = hash(cheie); //aflam pozitia listei ce contine elementul cu cheia "cheie"
